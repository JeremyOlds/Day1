...
function transformer(input) {
  ...
  let output = input.replaceAll("one", "o1e")
  output = output.replaceAll("two", "t2o")
  output = output.replaceAll("three", "t3e")
  output = output.replaceAll("four", "f4r")
  output = output.replaceAll("five", "f5e")
  output = output.replaceAll("six", "s6x")
  output = output.replaceAll("seven", "s7n")
  output = output.replaceAll("eight", "e8t")
  output = output.replaceAll("nine", "n9e")

  return output;

}

function findCalibration(input) {



  let transformed = transformer(input)

  array = transformed.split('')

  let numbers = array.filter(filtering)

  let arrayEnd = numbers.length

  let output = numbers[0]
  output += numbers[arrayEnd - 1]
...
function main() {
  ...
  let calibration = input.split('\n')

  calibration.forEach(cal => findCalibration(cal))
